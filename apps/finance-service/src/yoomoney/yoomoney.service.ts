...
export class YoomoneyServcie {
    ...
    async createPayoutProcess(body: any) {
        ...
        try {
            const { data: requestData } = await axios.post(`${providerSettings.baseURL}request-payment`, reqParams, {
                headers
            })

            if (requestData.status !== 'success') {
                transaction.status = PaymentTransactionStatus.FAILED;
                await this.financeTransactionRepo.getEntityManager().persistAndFlush(transaction);
                throw new BadRequestException(`request-payment failed: ${requestData.error}`);
            }

            const procParams = new URLSearchParams({ request_id: requestData.request_id });

            const { data: processData } = await axios.post(`${providerSettings.baseURL}process-payment`, procParams, {
                headers
            })
            ...
            if (processData.status === 'success') {
                ...
                throw new BadRequestException(`process-payment failed: ${processData.error}`);
            }
    ...
    async handleCallback(body: YooMoneyCallbackDto) {
        ...
        const transaction = await this.financeTransactionRepo.findOne({ id: Number(label) }, {
            populate: [
                'subMethod.method.providerSettings',
                'user.balance',
                'currency'
        ...
        if (transaction.status === PaymentTransactionStatus.COMPLETED ||
            transaction.status === PaymentTransactionStatus.FAILED) {
            throw new BadRequestException('Transaction already processed, return early');
        }
        ...
        const newGenerateSign = this.generateSignature(body, transaction.subMethod.method.providerSettings.privateKey as string)
        ...
        transaction.user.balance.balance += Number(amount)
    ...
    private generateSignature(body: YooMoneyCallbackDto, seckretKey: string): string {
        const { notification_type, operation_id, amount, currency, datetime, sender, codepro, label } = body
