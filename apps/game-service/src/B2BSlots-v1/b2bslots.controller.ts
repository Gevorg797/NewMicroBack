...
export class B2BSlotsController {
    ...
    loadGames(@Payload() payload: { userId: number; siteId: number; params?: any }) {
    ...
    initGameDemoSession(@Payload() payload: { userId: number; siteId: number; params: any }) {
        return this.service.initGameDemoSession(payload);
    }

    @MessagePattern('b2bslots.initGameSession')
    initGameSession(@Payload() payload: { userId: number; siteId: number; params: any }) {
        return this.service.initGameSession(payload);
    }

    @MessagePattern('b2bslots.gamesFreeRoundsInfo')
    gamesFreeRoundsInfo(@Payload() payload: { userId: number; siteId: number; params: any }) {
