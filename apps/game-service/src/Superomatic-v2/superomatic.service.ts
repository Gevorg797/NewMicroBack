...
export class SuperomaticService {
    ...
    async initGameDemoSession(payload: { userId: number; siteId: number; params: any }) {
        const { params, siteId } = payload;
        ...
        const response = await this.api.getGameDemoSession(baseURL, { ...params, sign });
        return response;
    }

    async initGameSession(payload: { userId: number; siteId: number; params: any }) {
        const { params, siteId } = payload;
        const { baseURL, key } = await this.providerSettings.getProviderSettings(siteId);
        const sign = this.utils.generateSigniture(
            params,
        ...
        const response = await this.api.getGameSession(baseURL, { ...params, sign });
        return response;
    }

    async gamesFreeRoundsInfo(payload: { userId: number; siteId: number; params: any }) {
        const { params, siteId } = payload;
        const { baseURL, key } = await this.providerSettings.getProviderSettings(siteId);
