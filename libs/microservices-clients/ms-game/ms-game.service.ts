...
export class MsGameService {
    ...
    superomaticInitDemo(data: { userId: number; siteId: number; params: any }) {
        return firstValueFrom(this.client.send('superomatic.initGameDemoSession', data));
    }
    superomaticInitSession(data: { userId: number; siteId: number; params: any }) {
        return firstValueFrom(this.client.send('superomatic.initGameSession', data));
    }
    superomaticFreeRoundsInfo(data: { userId: number; siteId: number; params: any }) {
        return firstValueFrom(this.client.send('superomatic.gamesFreeRoundsInfo', data));
    }
    ...
    b2bslotsInitDemo(data: { userId: number; siteId: number; params: any }) {
        return firstValueFrom(this.client.send('b2bslots.initGameDemoSession', data));
    }
    ...
    b2bslotsFreeRoundsInfo(data: { userId: number; siteId: number; params: any }) {
        return firstValueFrom(this.client.send('b2bslots.gamesFreeRoundsInfo', data));
    }
}
